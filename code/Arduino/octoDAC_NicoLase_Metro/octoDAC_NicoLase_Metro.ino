 ...
 * a BYTE;LONG;SHORT - append waveform state to queue
            ...
            Memory allocated for maxWaveformLength points total for all channels.
...
const int maxWaveformLength = 20000;
...
volatile unsigned int ampArray[maxWaveformLength] = {0};
volatile unsigned long timeArray[maxWaveformLength] = {0};
volatile byte chanArray[maxWaveformLength] = {0};
...
SPIClass xSPI (&sercom3, 12, 13, 11, SPI_PAD_3_SCK_1, SERCOM_RX_PAD_1);
...
const int inputStringLength = 10;
...
void setup() {
  ...
  xSPI.begin();
  pinPeripheral(11, PIO_SERCOM_ALT); // MISO
  pinPeripheral(12, PIO_SERCOM_ALT); // MOSI
  pinPeripheral(13, PIO_SERCOM_ALT); // SCK

  // Software reset of DAC
  // 0XXX 0111 XXXX XXXX XXXX XXXX XXXX XXXX
  xSPI.beginTransaction(SPISettings(24000000, MSBFIRST, SPI_MODE1));
  digitalWrite(syncPin,LOW);
  xSPI.transfer(0b00000111);
  xSPI.transfer(0b00000000); 
  xSPI.transfer(0b00000000); 
  xSPI.transfer(0b00000000); 
  ...
  digitalWrite(syncPin,LOW);
  xSPI.transfer(0b00001001);
  xSPI.transfer(0b00001010); 
  xSPI.transfer(0b00000000); 
  xSPI.transfer(0b00000000); 
  digitalWrite(syncPin,HIGH);
  ...
  // Start clear out array and associated variables
...
// Metro pin config values
  PORT->Group[PORTA].DIRSET.reg = PORT_PA02; // A0
  PORT->Group[PORTA].DIRSET.reg = PORT_PA05; // A1
  PORT->Group[PORTA].DIRSET.reg = PORT_PA06; // A2
  PORT->Group[PORTA].DIRSET.reg = PORT_PA04; // A3
  PORT->Group[PORTB].DIRSET.reg = PORT_PB08; // A4
  PORT->Group[PORTB].DIRSET.reg = PORT_PB09; // A5
...
void loop() {
 ...
 // Check if button has been pressed
  ...
  //--------------------------------------
  // Do things based on serial input
...
void ToggleLED() {
   ...
   if (LEDToggle) {
      togglePinsMetro(seqArray[cycleNum]);
...
//-----------------------------------------------------------
// Pin toggle for A0 - A5 on Metro M4
void togglePinsMetro(byte pinSeries){

//  Serial.println(pinSeries);

      if (0B00000001 & pinSeries){
      ...
      if (0B00000100 & pinSeries){
        PORT->Group[PORTA].OUTSET.reg = PORT_PA06;
      }
      else{
        PORT->Group[PORTA].OUTCLR.reg = PORT_PA06;
      } 

      if (0B00001000 & pinSeries){
        PORT->Group[PORTA].OUTSET.reg = PORT_PA04;
      }
      else{
        PORT->Group[PORTA].OUTCLR.reg = PORT_PA04;
      } 

      if (0B00010000 & pinSeries){
      ...
      if (0B00100000 & pinSeries){
        PORT->Group[PORTB].OUTSET.reg = PORT_PB09;
      }
      else{
        PORT->Group[PORTB].OUTCLR.reg = PORT_PB09;
      } 
